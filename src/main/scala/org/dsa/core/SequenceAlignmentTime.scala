...
/**
  ...
  */
abstract class SequenceAlignmentTime extends SequenceAlignment {
  ...
  def runWithIime(subMatrix: String, queryFile: String, dbFile: String, splitNum: Int, taskNum: Int, topK: Int) {
    ...
    val result = run(subMatrix, queryFile, dbFile, splitNum, taskNum, topK, sc)
    //     set application name
    //    val conf = new SparkConf().setAppName(appName)
