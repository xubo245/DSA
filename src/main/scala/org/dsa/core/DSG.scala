...
/**
  ...
  */
class DSG extends SequenceAlignment {
  /**
  ...
  override def align(query: (String, String), refRDD: RDD[(String, String)], scoreMatrix: Array[Array[Int]], topK: Int, sc: SparkContext): AlignmentRecordTopK = {
    ...
    val mapRDD = refRDD.map { ref =>
      ...
      val alignment = ParasailScala.alignSG(query._2, ref._2, 12, 2, Matrix.blosum50)
...
object DSG {
  ...
  def main(args: Array[String]) {
    ...
    val result = dsg.run(subMatrix: String, queryFile: String, dbFile: String, splitNum: Int, taskNum: Int, topK: Int, spark)
  ...
  def saveResult(str: String): Unit = {
    ...
    val conf = new SparkConf().setAppName("DSG" + " Application:saveResult,out:" + outStr)
    // initialize SparkContext
