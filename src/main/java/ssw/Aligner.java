...
public class Aligner {
    ...
    public static int[] lookup = new int[257]; // lookup[256] is used as sentinal for number of unique bases/matrix size
    ...
    public static Alignment alignNew(byte[] readNum, byte[] ref, int[][] matrix, int gapOpen, int gapExtend, boolean ignoreCase) {
        if (gapOpen < 0 || gapExtend < 0)
            throw new IllegalArgumentException("Gap open and extension penalties must be positive");
        if (gapOpen >= 256 || gapExtend >= 256)
            throw new IllegalArgumentException("Gap open and extension penalties must fit into unsigned 8-bit integer");
...
//        lookup[256] = 0;
    ...
    public static byte[] profile(byte[] read, boolean ignoreCase) {
//        int[] lookup = new int[257]; // lookup[256] is used as sentinal for number of unique bases/matrix size
        java.util.Arrays.fill(lookup, -1);
        lookup[256] = 0;
        byte[] readNum = convertToNumeric(lookup, read, ignoreCase);
